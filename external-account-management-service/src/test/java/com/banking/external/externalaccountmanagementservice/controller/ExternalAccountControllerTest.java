...
public class ExternalAccountControllerTest {
	...
	@Test
	public void testDeleteBeneficiary_whenBeneficiaryNotExists() {
		Mockito.doThrow(EmptyResultDataAccessException.class).when(service).deleteBeneficiary(anyLong());
		ResponseEntity<HttpStatus> response = controller.deleteBeneficiary(1L);
		assertEquals(new ResponseEntity<HttpStatus>(HttpStatus.NOT_FOUND), response);
	}
	...
	public void testFetchDefault() {
		List<AccountBean> accounts = controller.fetchDefault();
		assertNotNull(accounts);
